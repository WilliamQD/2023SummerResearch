...
```{r}
GB <- read.csv("./radNet/radnet-GB-new.csv")
potassium40 <- read.csv("./radNet/radnet-potassium40.csv")
radium228 <- read.csv("./radNet/radnet-radium228.csv")
```

# radium-228
```{r, include=FALSE}
radium228 %>% filter(Location == "CARLSBAD")
```

# potassium-40
```{r, include=FALSE}
potassium40 %>% filter(Location == "CARLSBAD")
```
...
```
```{r}
ggplot(GB_monthly, aes(Date, mean_result)) +
  geom_point()
```
...
```
```{r}
GB_daily_complete <- merge(x = GB_each,
                          y = pb_vnf_daily,
                          by.x = "date",
                          by.y = "date",
                          all.x = FALSE,
...
summary(lm_gb)
gam_gb <- gam(Result ~ date + distToCarls + temp_bb + rhi + area_bb + mean_solar + mean_temp + mean_ws + oil + gas +month_count + day_count , data=GB_daily_complete)
summary(gam_gb)
```
```{r}
gam_gb_v2 <- gam(Result ~ s(as.numeric(date), bs="cr") + distToCarls + temp_bb + rhi + area_bb + mean_solar + mean_temp + mean_ws + s(oil, bs="cr") + s(gas, bs="cr") + month_count + day_count , data=GB_daily_complete)
summary(gam_gb_v2)
```
